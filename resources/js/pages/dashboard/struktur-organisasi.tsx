...
function PersonAvatar({ person, size = 'size-10 text-xs' }: { person: Person; size?: string }) {
    return (
        <div
            className={cn(
        ...
        >
            {person.foto_url ? (
                <img src={person.foto_url} alt={person.nama} loading="lazy" className="h-full w-full object-cover" />
            ) : (
    ...
    return (
        <div>
            ...
            <div className="flex items-center gap-4">
                <div className="flex size-20 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-orange-100 to-yellow-100 dark:from-orange-900/40 dark:to-yellow-900/30">
                    {src ? (
                        <img src={src} alt="Preview Foto" className="h-full w-full object-cover" />
                    ) : (
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-[10px] text-neutral-400 dark:text-neutral-500">
                        JPG / PNG / GIF / WebP, maks 5 MB. Otomatis dikonversi ke WebP tanpa pengurangan kualitas.
                    </p>
                </div>
            </div>
        </div>
...
function ModalShell({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-yellow-100/40 bg-white shadow-2xl dark:border-neutral-700/40 dark:bg-neutral-900">
...
function AnggotaModal({
    ...
    const needsBidang = data.kategori === 'ketua_bidang' || data.kategori === 'anggota';
    ...
    return (
        <ModalShell title={isEdit ? 'Edit Anggota' : 'Tambah Anggota'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4 px-6 py-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="AR"
                    />
                    {errors.inisial && <p className="mt-1 text-xs text-red-500">{errors.inisial}</p>}
                </div>
                ...
                {needsBidang && (
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setData('bidang_id', e.target.value ? Number(e.target.value) : null)}
                            className={inputClass}
                        >
                        ...
                        {errors.bidang_id && <p className="mt-1 text-xs text-red-500">{errors.bidang_id}</p>}
                        {needsBidang && !data.bidang_id && (
                            <p className="mt-1 text-xs text-red-400">Bidang wajib diisi untuk anggota ini.</p>
                        )}
                    </div>
                )}
...
function BidangModal({
    ...
    return (
        <ModalShell title={isEdit ? 'Edit Bidang' : 'Tambah Bidang'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4 px-6 py-4">
                <div>
                    <label className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                        Nama Bidang
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-orange-100 dark:bg-orange-900/30">
                            {SelectedIcon && <SelectedIcon className="size-5 text-orange-500 dark:text-orange-400" />}
                        </div>
                        <select
                            value={data.icon_key}
                    ...
                    {errors.icon_key && <p className="mt-1 text-xs text-red-500">{errors.icon_key}</p>}
                </div>
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <button
                        ...
                        className="flex-1 rounded-xl bg-orange-400 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-orange-500 disabled:opacity-50 dark:bg-orange-500 dark:hover:bg-orange-600"
                    >
                        {processing ? 'Menyimpan...' : isEdit ? 'Simpan Perubahan' : 'Tambah Bidang'}
                    </button>
                </div>
            </form>
        </ModalShell>
...
function KontakModal({
    ...
    return (
        <ModalShell title={isEdit ? 'Edit Kontak' : 'Tambah Kontak'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4 px-6 py-4">
                <div>
                    <label className="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                        Nama
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <button
                        ...
                        className="flex-1 rounded-xl bg-orange-400 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-orange-500 disabled:opacity-50 dark:bg-orange-500 dark:hover:bg-orange-600"
                    >
                        {processing ? 'Menyimpan...' : isEdit ? 'Simpan Perubahan' : 'Tambah Kontak'}
                    </button>
                </div>
            </form>
        </ModalShell>
...
export default function StrukturOrganisasi({
    ...
    const [anggotaModal, setAnggotaModal] = useState<{
        ...
        person: Person | null;
    }>({ open: false, kategori: 'pembina_penasihat', bidangId: null, person: null });
    const [bidangModal, setBidangModal] = useState<{ open: boolean; bidang: Bidang | null }>({
        open: false,
        bidang: null,
    });
    const [kontakModal, setKontakModal] = useState<{ open: boolean; kontak: Kontak | null }>({
        open: false,
        kontak: null,
    });

    const openAnggota = (kategori: string, bidangId: number | null, person: Person | null) => {
        setAnggotaModal({ open: true, kategori, bidangId, person });
    };

    const handleDeletePerson = (person: Person) => {
        if (confirm(`Yakin ingin menghapus ${person.nama}?`)) {
            router.delete(`/dashboard/struktur-organisasi/anggota/${person.id}`);
        }
    };

    const handleDeleteBidang = (bidang: Bidang) => {
        if (confirm(`Yakin ingin menghapus ${bidang.nama} beserta seluruh anggotanya?`)) {
    ...
    const editBtns = (onEdit: () => void, onDelete: () => void) => (
        <div className="flex shrink-0 items-center gap-2">
            <ActionButton title="Edit" onClick={onEdit} className="border-neutral-200 bg-white text-neutral-600 hover:bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">
                <Pencil className="size-3.5" />
            </ActionButton>
            <ActionButton title="Hapus" onClick={onDelete} className="border-red-200 bg-red-50 text-red-600 hover:bg-red-100 dark:border-red-800/50 dark:bg-red-900/20 dark:text-red-400">
                <Trash2 className="size-3.5" />
            </ActionButton>
        </div>
    );
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-neutral-600 dark:text-neutral-400">
                                    Kelola susunan kepengurusan PARI Pengda Jambi
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                {tab === 'pembina' && (
                    <div className="grid gap-4 lg:grid-cols-2">
                        {/* Ketua Umum */}
                        <div className="rounded-2xl border border-yellow-100/40 bg-yellow-50/80 backdrop-blur-md dark:border-neutral-700/40 dark:bg-neutral-800/80">
                            <div className="flex items-center justify-between border-b border-yellow-100/40 px-6 py-4 dark:border-neutral-700/40">
                                ...
                                {chairman && (
                                    <button
                                        type="button"
                                        onClick={() => openAnggota('ketua_umum', null, chairman)}
                            ...
                            <div className="p-6">
                                {chairman ? (
                                    <div className="flex items-center gap-4">
                                        <PersonAvatar person={chairman} size="size-16 text-lg" />
                                        ...
                                        <ActionButton
                                            title="Hapus"
                                            onClick={() => handleDeletePerson(chairman)}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => openAnggota('ketua_umum', null, null)}
                        ...
                        <div className="rounded-2xl border border-yellow-100/40 bg-yellow-50/80 backdrop-blur-md dark:border-neutral-700/40 dark:bg-neutral-800/80">
                            <div className="flex items-center justify-between border-b border-yellow-100/40 px-6 py-4 dark:border-neutral-700/40">
                                <h2 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">
                                    Daftar Pembina & Penasihat ({pembinas.length})
                                </h2>
                                <button
                                    type="button"
                                    onClick={() => openAnggota('pembina_penasihat', null, null)}
                                ...
                                {pembinas.length === 0 ? (
                                    ...
                                    pembinas.map((person) => (
                                        <div key={person.id} className="flex items-center gap-3 px-6 py-3">
                                            ...
                                            {editBtns(
                                                () => openAnggota('pembina_penasihat', null, person),
                ...
                {tab === 'bidang' && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                type="button"
                                onClick={() => setBidangModal({ open: true, bidang: null })}
                                className="inline-flex items-center gap-2 rounded-xl bg-orange-400 px-4 py-2 text-sm font-bold text-white transition hover:bg-orange-500 dark:bg-orange-500 dark:hover:bg-orange-600"
                            >
                                <Plus className="size-4" />
                                Tambah Bidang
                        ...
                        {bidangs.length === 0 && (
                            <div className="rounded-2xl border border-yellow-100/40 bg-yellow-50/80 px-6 py-12 text-center text-sm text-neutral-500 dark:border-neutral-700/40 dark:bg-neutral-800/80 dark:text-neutral-400">
                                Belum ada bidang.
                            </div>
                        )}

                        {bidangs.map((bidang) => {
                            const Icon = strukturIconMap[(bidang.icon_key || 'users') as StrukturIconKey] ?? Users;

                            return (
                                <div
                                    key={bidang.id}
                                    className="overflow-hidden rounded-2xl border border-yellow-100/40 bg-yellow-50/80 backdrop-blur-md dark:border-neutral-700/40 dark:bg-neutral-800/80"
                                >
                                    {/* Bidang header */}
                                    <div className="flex items-center justify-between border-b border-yellow-100/40 px-6 py-4 dark:border-neutral-700/40">
                                        <div className="flex items-center gap-3">
                                            <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-orange-100 dark:bg-orange-900/30">
                                                {Icon && <Icon className="size-4 text-orange-500 dark:text-orange-400" />}
                                            </div>
                                            <div>
                                        ...
                                        {editBtns(
                                            () => setBidangModal({ open: true, bidang }),
                                    ...
                                    <div className="border-b border-dashed border-orange-200 px-6 py-4 dark:border-orange-800/40">
                                        <div className="mb-3 flex items-center justify-between">
                                            ...
                                            {!bidang.ketua && (
                                                <button
                                                    type="button"
                                                    onClick={() => openAnggota('ketua_bidang', bidang.id, null)}
                                        ...
                                        {bidang.ketua ? (
                                            <div className="flex items-center gap-3">
                                                <PersonAvatar person={bidang.ketua} />
                                                <div className="min-w-0 flex-1">
                                                    <p className="truncate text-sm font-bold text-neutral-800 dark:text-neutral-200">
                                                        {bidang.ketua.nama}
                                                    </p>
                                                ...
                                                {editBtns(
                                                    () => openAnggota('ketua_bidang', bidang.id, bidang.ketua),
                                                    () => handleDeletePerson(bidang.ketua as Person),
                                                )}
                                            </div>
                                        ) : (
                                    ...
                                    <div className="px-6 py-4">
                                        <div className="mb-3 flex items-center justify-between">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => openAnggota('anggota', bidang.id, null)}
                                                className="inline-flex items-center gap-1.5 rounded-lg border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs font-semibold text-orange-600 transition hover:bg-orange-100 dark:border-orange-800/50 dark:bg-orange-900/20 dark:text-orange-400"
                                            >
                                                <Plus className="size-3.5" />
                                                Tambah Anggota
                                        ...
                                        {bidang.anggota.length === 0 ? (
                                            ...
                                            <div className="divide-y divide-neutral-100 dark:divide-neutral-700/40">
                                                {bidang.anggota.map((anggota) => (
                                                    <div key={anggota.id} className="flex items-center gap-3 py-2.5">
                                                        <PersonAvatar person={anggota} size="size-9 text-[10px]" />
                                                        <p className="min-w-0 flex-1 truncate text-sm font-medium text-neutral-800 dark:text-neutral-200">
                                                            {anggota.nama}
                                                        </p>
                                                        {editBtns(
                                                            () => openAnggota('anggota', bidang.id, anggota),
                                                            () => handleDeletePerson(anggota),
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                ...
                {tab === 'kontak' && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                type="button"
                                onClick={() => setKontakModal({ open: true, kontak: null })}
                                className="inline-flex items-center gap-2 rounded-xl bg-orange-400 px-4 py-2 text-sm font-bold text-white transition hover:bg-orange-500 dark:bg-orange-500 dark:hover:bg-orange-600"
                            >
                                <Plus className="size-4" />
                                Tambah Kontak
                        ...
                        <div className="overflow-hidden rounded-2xl border border-yellow-100/40 bg-yellow-50/80 backdrop-blur-md dark:border-neutral-700/40 dark:bg-neutral-800/80">
                            {kontaks.length === 0 ? (
                                ...
                                <div className="divide-y divide-neutral-100 dark:divide-neutral-700/40">
                                    {kontaks.map((kontak) => (
                                        <div key={kontak.id} className="flex items-center gap-3 px-6 py-4">
                                            ...
                                            {editBtns(
                                                () => setKontakModal({ open: true, kontak }),
                ...
                <div className="flex items-center gap-2 text-[11px] text-neutral-400 dark:text-neutral-500">
                    ...
                    Data akan otomatis tampil di halaman publik Profil {'>'} Struktur Organisasi. Foto diupload otomatis
                    menjadi WebP tanpa pengurangan kualitas.
                </div>
            </div>
            ...
            <AnggotaModal
                ...
                bidangs={bidangs}
                onClose={() => setAnggotaModal((s) => ({ ...s, open: false, person: null }))}
            />
            <BidangModal
